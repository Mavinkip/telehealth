...
class AuthManager {
    ...
    async register(email, password, fullName, role, phone = '', specialty = '') {
        ...
        const { data: authData, error: authError } = await supabase.auth.signUp({
            ...
            options: {
                data: {
                    full_name: fullName,
                    role,
                    // Admins and patients start active; doctors start inactive
        ...
        if (role === 'admin') {
            return {
                success: true,
                message: '🎉 Admin account created successfully! You now have full system access. Please log in.'
            };
        }
    ...
    showRegisterPage() {
        ...
        document.getElementById('registerForm').addEventListener('submit', async e => {
            ...
            if (result.success) {
                // If admin created, redirect to login after 3 seconds
                if (role === 'admin') {
