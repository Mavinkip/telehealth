/*
 * File: doctor.js - Complete Doctor Manager with FIXED Navigation & Profile
 */
...
class DoctorManager {
    ...
    // =============================================
    async loadView(view) {
        console.log('📱 Doctor loading view:', view);
        
        // Use app-content from app.js
        let content = document.getElementById('doctorContent');
        if (!content) {
        ...
        try {
            ...
            content.innerHTML = `
                ...
                <button class="btn btn-primary mt-3" onclick="doctorManager.goToDashboard()">⬅️ Back to Dashboard</button>
            `;
        }
    }
    ...
    async loadDashboardContent(container) {
        ...
        container.innerHTML = `
            ...
            <div class="stats-grid">
                <div class="stat-card" onclick="doctorManager.goToAppointments()" style="cursor:pointer;">
                    <div class="stat-label">📅 Today's Appointments</div>
                    <div class="stat-value accent">${todayAppointments?.length || 0}</div>
                </div>
                <div class="stat-card" onclick="doctorManager.goToPatients()" style="cursor:pointer;">
                ...
                <div class="stat-card" onclick="doctorManager.goToChat()" style="cursor:pointer;">
        ...
        return `
            <div class="appointment-card">
                ...
                <div class="appointment-actions">
                    ...
                    <button class="btn btn-sm btn-info" onclick="doctorManager.goToChat()">
                        💬 Message
                    </button>
        ...
        container.innerHTML = `
            <div class="row">
                <div class="col-12">
                    ...
                    <button class="btn btn-outline-secondary mb-3" onclick="doctorManager.goToDashboard()">⬅️ Dashboard</button>
                </div>
            </div>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        ${apt.status === 'scheduled' ? `
                                                            ...
                                                            <button class="btn btn-sm btn-info" onclick="doctorManager.goToChat()">💬</button>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            ...
                            <span class="badge bg-primary">${chatPartners.length}</span>
                        </div>
                        <div class="card-body">
    ...
    async loadProfileContent(container) {
                ...
                <div class="col-md-4">
                    <div class="card">
                        <div class="card-body text-center">
                            ...
                            <hr>
                            <button class="btn btn-outline-primary w-100 mt-2" onclick="doctorManager.goToAppointments()">
                                📅 Appointments
                            </button>
                            <button class="btn btn-outline-info w-100 mt-2" onclick="doctorManager.goToChat()">
                                💬 Messages
                            </button>
                            <button class="btn btn-outline-success w-100 mt-2" onclick="doctorManager.goToPatients()">
                                👥 Patients
                            </button>
                            <button class="btn btn-outline-secondary w-100 mt-2" onclick="doctorManager.goToDashboard()">
                                📊 Dashboard
                            </button>
                        </div>
                    </div>
        ...
        if (profileForm) {
            ...
            console.warn('⚠️ Doctor profile form not found');
        }
    }
    ...
    showPrescriptionModal(patientId, patientName) {
        ...
        overlay.innerHTML = `
            <div style="background: #ffffff; border-radius: 16px; padding: 30px 32px; max-width: 600px; width: 100%; max-height: 90vh; overflow-y: auto; box-shadow: 0 20px 60px rgba(0,0,0,0.3); position: relative; border: 1px solid #e2e8f0;">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 2px solid #10B981; padding-bottom: 14px; margin-bottom: 18px;">
                    <div>
                        <h5 style="font-size: 1.25rem; font-weight: 700; margin: 0; color: #0F172A;">💊 Write Prescription</h5>
                        <small style="display: block; font-size: 0.85rem; color: #64748B; margin-top: 2px;">For: <strong>${patientName || 'Patient'}</strong></small>
                    </div>
                    <button onclick="document.getElementById('prescriptionModal').remove()" style="background: none; border: none; font-size: 1.8rem; cursor: pointer; color: #94A3B8; padding: 0 6px;">×</button>
                </div>
                ...
                <form id="prescriptionForm">
                    ...
                    <div style="margin-bottom: 14px;">
                        <label style="display:block;font-weight:600;margin-bottom:4px;font-size:0.85rem;color:#0F172A;">💊 Medication Name *</label>
                        <input type="text" id="medicationName" placeholder="e.g., Amoxicillin, Lisinopril, Metformin" style="width:100%;padding:10px 14px;border:2px solid #E2E8F0;border-radius:8px;font-size:0.95rem;background:#F8FAFC;">
                    </div>

                    <div style="margin-bottom: 14px;">
                        <label style="display:block;font-weight:600;margin-bottom:4px;font-size:0.85rem;color:#0F172A;">📏 Dosage & Strength *</label>
                        <input type="text" id="dosage" placeholder="e.g., 500mg, 10mg, 25mg/5ml" style="width:100%;padding:10px 14px;border:2px solid #E2E8F0;border-radius:8px;font-size:0.95rem;background:#F8FAFC;">
                    </div>

                    <div style="display:flex;gap:12px;margin-bottom:14px;flex-wrap:wrap;">
                        <div style="flex:1;min-width:140px;">
                            <label style="display:block;font-weight:600;margin-bottom:4px;font-size:0.85rem;color:#0F172A;">⏰ Frequency *</label>
                            <select id="frequency" style="width:100%;padding:10px 14px;border:2px solid #E2E8F0;border-radius:8px;font-size:0.95rem;background:#F8FAFC;">
                        ...
                        <div style="flex:1;min-width:140px;">
                            <label style="display:block;font-weight:600;margin-bottom:4px;font-size:0.85rem;color:#0F172A;">📅 Duration *</label>
                            <select id="duration" style="width:100%;padding:10px 14px;border:2px solid #E2E8F0;border-radius:8px;font-size:0.95rem;background:#F8FAFC;">
                    ...
                    <div style="margin-bottom:14px;">
                        <label style="display:block;font-weight:600;margin-bottom:4px;font-size:0.85rem;color:#0F172A;">🍽️ When to Take</label>
                        <select id="whenToTake" style="width:100%;padding:10px 14px;border:2px solid #E2E8F0;border-radius:8px;font-size:0.95rem;background:#F8FAFC;">
                    ...
                    <div style="margin-bottom:14px;">
                        <label style="display:block;font-weight:600;margin-bottom:4px;font-size:0.85rem;color:#0F172A;">📝 Special Instructions</label>
                        <textarea id="instructions" rows="2" placeholder="e.g., Take with plenty of water, Avoid alcohol..." style="width:100%;padding:10px 14px;border:2px solid #E2E8F0;border-radius:8px;font-size:0.95rem;background:#F8FAFC;resize:vertical;"></textarea>
                    </div>

                    <div style="margin-bottom:14px;">
                        <label style="display:block;font-weight:600;margin-bottom:4px;font-size:0.85rem;color:#0F172A;">📝 SOAP Notes (Medical Record)</label>
                        <textarea id="soapNotes" rows="3" placeholder="Enter clinical notes, diagnosis, treatment plan..." style="width:100%;padding:10px 14px;border:2px solid #E2E8F0;border-radius:8px;font-size:0.95rem;background:#F8FAFC;resize:vertical;"></textarea>
                    </div>
                    ...
                    <div style="display:flex;align-items:center;gap:10px;margin:12px 0;padding:12px;background:#F8FAFC;border-radius:8px;border:1px solid #E2E8F0;">
                        ...
                        <label for="sendReminders" style="font-weight:500;font-size:0.9rem;cursor:pointer;color:#0F172A;">🔔 Send medication reminders to patient</label>
                    </div>

                    <button type="submit" style="width:100%;padding:13px;background:#10B981;color:white;border:none;border-radius:8px;font-size:1rem;font-weight:600;cursor:pointer;transition:0.3s;display:flex;align-items:center;justify-content:center;gap:8px;margin-top:4px;">
        ...
        if (medicationName) {
            medicationName.addEventListener('input', function() {
                const soapNotes = document.getElementById('soapNotes');
                if (this.value && soapNotes && !soapNotes.value) {
    ...
    async viewPatientHistory(patientId, patientName) {
        ...
        overlay.innerHTML = `
            <div style="background: white; border-radius: 16px; padding: 30px 32px; max-width: 750px; width: 100%; max-height: 90vh; overflow-y: auto; box-shadow: 0 20px 60px rgba(0,0,0,0.3); position: relative; border: 1px solid #e2e8f0;">
                <div style="display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #2563EB;padding-bottom:14px;margin-bottom:18px;">
                    <div>
                        <h5 style="font-size:1.25rem;font-weight:700;margin:0;color:#0F172A;">📄 Medical History</h5>
                        <small style="display:block;font-size:0.85rem;color:#64748B;margin-top:2px;">${patientName || 'Patient'}</small>
                    </div>
                    <button onclick="document.getElementById('historyModal').remove()" style="background:none;border:none;font-size:1.8rem;cursor:pointer;color:#94A3B8;padding:0 6px;">×</button>
                </div>
                    ...
                    html += `
                        <div style="border:1px solid #E2E8F0;border-radius:12px;margin-bottom:16px;overflow:hidden;border-left:4px solid #2563EB;">
                            <div style="background:#F8FAFC;padding:12px 16px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;">
                                <div><strong>👨‍⚕️ ${record.doctor?.full_name || 'Unknown Doctor'}</strong></div>
                                <small style="color:#64748B;">📅 ${new Date(record.created_at).toLocaleString()}</small>
                            </div>
                            <div style="padding:16px;">
                                <div style="margin-bottom:12px;">
                                    <strong>📝 SOAP Notes:</strong>
                                    <p style="background:#F8FAFC;padding:12px;border-radius:8px;margin-top:4px;">${record.soap_notes || 'No notes available'}</p>
                                </div>
                                ${record.diagnosis ? `<div style="margin-bottom:8px;"><strong>🩺 Diagnosis:</strong><p style="margin:4px 0 0 0;">${record.diagnosis}</p></div>` : ''}
                                ${record.treatment_plan ? `<div style="margin-bottom:8px;"><strong>📋 Treatment Plan:</strong><p style="margin:4px 0 0 0;">${record.treatment_plan}</p></div>` : ''}
                                ...
                                ` : `<div style="color:#64748B;font-size:0.85rem;margin-top:8px;">ℹ️ No prescriptions in this record</div>`}
                            </div>
                        </div>
                    `;
                }
